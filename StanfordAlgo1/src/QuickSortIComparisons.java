...
public class QuickSortIComparisons {
	...
	private static void quickSort(Integer[] integers, int start, int end) {
		if(start>=end)
			return;
		int ppos = getPivot(integers,start,end);
		int mid = partition(integers,ppos,start,end);
		comparisons+=mid-1-start+1;
		quickSort(integers,start,mid-1);
		comparisons+=end-mid-1+1;
	...
	private static int getPivot(Integer[] integers, int start, int end) {
